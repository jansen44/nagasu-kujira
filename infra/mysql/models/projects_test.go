...
func testProjectToManyMissions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, missionDBTypes, false, missionColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, missionDBTypes, false, missionColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Missions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.ProjectsID == b.ProjectsID {
			bFound = true
		}
		if v.ProjectsID == c.ProjectsID {
			cFound = true
		}
	}
	...
	if err = a.L.LoadMissions(ctx, tx, false, (*[]*Project)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Missions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Missions = nil
	if err = a.L.LoadMissions(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Missions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProjectToManyAddOpMissions(t *testing.T) {
	var err error
	...
	foreigners := []*Mission{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, missionDBTypes, false, strmangle.SetComplement(missionPrimaryKeyColumns, missionColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Mission{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddMissions(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.ID != first.ProjectsID {
			t.Error("foreign key was wrong value", a.ID, first.ProjectsID)
		}
		if a.ID != second.ProjectsID {
			t.Error("foreign key was wrong value", a.ID, second.ProjectsID)
		}
		...
		if a.R.Missions[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Missions[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Missions().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	projectDBTypes = map[string]string{`ID`: `int`, `Name`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_              = bytes.MinRead
)
