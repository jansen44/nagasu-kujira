...
func testTaskToOneMissionUsingMission(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadMission(ctx, tx, false, (*[]*Task)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
var (
	taskDBTypes = map[string]string{`ID`: `int`, `MissionID`: `int`, `Name`: `varchar`, `Description`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_           = bytes.MinRead
)
