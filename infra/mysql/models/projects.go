...
type projectR struct {
	Missions MissionSlice `boil:"Missions" json:"Missions" toml:"Missions" yaml:"Missions"`
}
...
var (
	projectAllColumns            = []string{"id", "name", "created_at", "updated_at"}
...
// Missions retrieves all the mission's Missions with an executor.
func (o *Project) Missions(mods ...qm.QueryMod) missionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`missions`.`projects_id`=?", o.ID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"`missions`.*"})
	}

	return query
}

// LoadMissions allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (projectL) LoadMissions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProject interface{}, mods queries.Applicator) error {
	var slice []*Project
	var object *Project
	...
	query := NewQuery(
		qm.From(`missions`),
		qm.WhereIn(`missions.projects_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load missions")
	}

	var resultSlice []*Mission
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice missions")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on missions")
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.ProjectsID {
				local.R.Missions = append(local.R.Missions, foreign)
				if foreign.R == nil {
					foreign.R = &missionR{}
				}
...
// AddMissions adds the given related objects to the existing relationships
// of the project, optionally inserting them as new records.
// Appends related to o.R.Missions.
// Sets related.R.Project appropriately.
func (o *Project) AddMissions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Mission) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.Missions = append(o.R.Missions, related...)
	}
